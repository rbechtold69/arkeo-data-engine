...
echo ""
echo -e "${CYAN}${BOLD}╔══════════════════════════════════════════╗${NC}"
echo -e "${CYAN}${BOLD}║     🛡️  Arkeo Sentinel Installer         ║${NC}"
echo -e "${CYAN}${BOLD}╚══════════════════════════════════════════╝${NC}"
echo ""
...
echo -e "  ${GREEN}✓${NC} Config saved to ~/.arkeo/config.yaml"
...
EOF
echo -e "  ${GREEN}✓${NC} Metadata saved to ~/.arkeo/metadata.json"

# ── Step 3: Set up HTTPS (if domain provided) ────────────────
if [ -n "$DOMAIN" ]; then
    ...
    if ! command -v nginx &>/dev/null; then
        ...
        echo -e "  ${GREEN}✓${NC} Nginx + Certbot installed"
    else
...
EOF
    ...
    # Get SSL cert
    echo -e "  ${YELLOW}→${NC} Getting SSL certificate (this may take a minute)..."
    certbot --nginx -d "${DOMAIN}" --non-interactive --agree-tos --register-unsafely-without-email --redirect > /dev/null 2>&1 && \
        echo -e "  ${GREEN}✓${NC} HTTPS configured with auto-renewal" || \
        echo -e "  ${YELLOW}⚠${NC} SSL setup failed — sentinel will work on HTTP. Run 'certbot --nginx -d ${DOMAIN}' manually."
else
    echo -e "${CYAN}[3/5]${NC} Skipping HTTPS (no domain provided)"
    echo -e "  ${YELLOW}ℹ${NC} Sentinel will be available at http://YOUR_IP:3636"
    echo -e "  ${YELLOW}ℹ${NC} Add a domain later for HTTPS (recommended)"
fi
...
if docker ps | grep -q arkeo-sentinel; then
    echo -e "  ${GREEN}✓${NC} Sentinel is running"
else
    echo -e "  ${RED}✗${NC} Sentinel failed to start. Check logs with: docker logs arkeo-sentinel"
    exit 1
fi
...
if echo "$HEALTH" | grep -q "moniker"; then
    echo -e "  ${GREEN}✓${NC} Sentinel responding locally"
else
    echo -e "  ${YELLOW}⚠${NC} Sentinel not responding yet — it may need a few more seconds to start"
fi
...
if [ -n "$DOMAIN" ]; then
    ...
    if echo "$PUBLIC" | grep -q "moniker"; then
        echo -e "  ${GREEN}✓${NC} Public HTTPS endpoint working"
    else
        echo -e "  ${YELLOW}⚠${NC} Public endpoint not responding yet — DNS may need a few minutes to propagate"
    fi
fi
...
echo -e "${GREEN}${BOLD}║     🚀  Sentinel is Live!                ║${NC}"
echo -e "${GREEN}${BOLD}╚══════════════════════════════════════════╝${NC}"
echo ""
if [ -n "$DOMAIN" ]; then
